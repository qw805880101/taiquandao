...
export default class SiderMenu extends PureComponent {
   ...
   * get SubMenu or Item
   */
  getSubMenuOrItem=(item, onMenuClick) => {
    if (item.children && item.children.some(child => child.name)) {
      return (
        ...
        >
          {this.getNavMenuItems(item.children, onMenuClick)}
        </SubMenu>
      );
    } else {
      return (
   ...
   * @memberof SiderMenu
   */
  getNavMenuItems = (menusData, onMenuClick) => {
    if (!menusData) {
      return [];
    }
    return menusData
      ...
      .map((item) => {
        const ItemDom = this.getSubMenuOrItem(item, onMenuClick);
  ...
  render() {
    const { logo, collapsed, location: { pathname }, onCollapse ,menuData, onMenuClick} = this.props;
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/*{this.getNavMenuItems(menuData, onMenuClick)}*/}
        </Menu>
      </Sider>
    );
  }
